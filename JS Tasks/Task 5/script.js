...
// filterDogOwners function
function filterDogOwners(array) {
  const owners = array.filter((dogOwners) => dogOwners.hasDog);
  return owners;
}
const dogOwners = filterDogOwners(users);
console.log("Dog Owners Array: ", dogOwners);

// 2 užduotis

// function filterAdults
function filterAdults(array) {
  return array.filter((users) => users.age > 17);
}
